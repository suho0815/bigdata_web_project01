...
## 2023/08/11
- 카카오맵 API이용 동물병원 위치 
    - 원본 데이터에 위도, 경도 데이터가 있어서 기능을 구현했는데
    - 다시 데이터를 확인해서보니 위도와 경도가 잘못 표기 되어 있다..
    - 아예 사용이 불가능하다.
    - 데이터 확인을 제대로 하지 못해서 나온 결과다... 
// 데이터셋을 이용하기 전에는 꼭 확인하자..

- 2개의 테이블 좋아요 순으로 뽑아내기
    - join view 생성 방법(쿼리문)
        - CREATE VIEW '이름' AS
        - SELECT '테이블1' AS source, 필드1, 필드2 .... FROM 테이블1
        - UNION ALL
        - SELECT '테이블2' AS source, 필드1, 필드2 .... FROM 테이블2;
    - view 엔티티 생성(읽기 전문)
// 새롭게 알게된 사실이다.
...
- 궁금한 것
    ...
    - @SqlResultSetMapping ??
    - JPQL과 JPA 차이점
    - 네이티브쿼리, 쿼리메소드 , JPQL, JPA
    - 낙타표기법 > 뱀표기법
        - SpringBoot > MySQL 진행될 때 표기법이 자동으로 바뀐다. 
        - SpringBoot 책을 보면 정확하게 나와 있다.